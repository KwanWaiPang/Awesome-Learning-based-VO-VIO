...
|2026|`CVPR`|[SCE-SLAM: Scale-Consistent Monocular SLAM via Scene Coordinate Embeddings](https://arxiv.org/pdf/2601.09665)|---|基于DPVO的双分支单目SLAM，用scene coordinate embeddings在canonical scale下维持跨窗口尺度一致性（非预测绝对metric尺度）：CNN加DINOv3提特征，按SuperPoint每帧采样patch；flow分支做像素级跟踪，scene coordinate分支以patch级表征累积尺度记忆；Geometry-Guided Scale Propagation用几何调制注意力从空间邻近的历史参考patch传播尺度，经GRU与帧内聚合更新；Scene Coordinate Bundle Adjustment将embedding解码的三维坐标先验作为世界系约束，与光流残差交替优化，把漂移拉回初始化尺度|
...
|2026|`arXiv`|[RADIO-ViPE: Online Tightly Coupled Multi-Modal Fusion for Open-Vocabulary Semantic SLAM in Dynamic Environments](https://arxiv.org/pdf/2604.26067)|[![Github stars](https://img.shields.io/github/stars/be2rlab/RADIO-ViPE.svg)](https://github.com/be2rlab/RADIO-ViPE)|[website](https://be2rlab.github.io/radio_vipe) <br> RADIO-ViPE = 基于DROID-SLAM的架构，在 ViPE 上做 视觉–语言–几何紧耦合 的在线 Semantic SLAM；输入只要未标定单目 RGB，就能建图、定位，并支持开放词汇 3D 查询。GeoCalib 进行相机的初始化（从均匀采样的帧中引导出相机内参，无需标定目标或已知相机模型） + RADSeg/RADIO 稠密特征 + 单目深度；在 BA 中联合光流几何项与RADIO embedding 相似项；时序自适应鲁棒核处理动态/准静态物体；SigLIP 做开放词汇 3D grounding；|
